# Source http://www3.inegi.org.mx/rnm/index.php/catalog/208

CensoMunicipal = function(fuente = NA, year = NA){
library(foreign)
  library(rio)
fformat = "dbf"
temp.cngmd = tempfile()
zipdir = tempfile()
### AYUNTAMIENTO - ESTRUCTURA AYUNTAMIENTOS -----------------------------------------------------------
if (fuente == "EST_AYUN" | fuente == "INTEGRAN" | fuente == "PER_APOY") {
  url.estructura  = paste0("http://www.beta.inegi.org.mx/contenidos/proyectos/censosgobierno/municipal/cngmd/2015/microdatos/m1/Estruct_aytto_cngmd2015_", fformat, ".zip")
  utils::download.file(url.estructura, temp.cngmd)
  utils::unzip(temp.cngmd, exdir = zipdir)

  if (fuente == "EST_AYUN") {
    raw  = paste0(zipdir, "\\Estruct_aytto_cngmd2015_dbf\\Bases_de_datos\\EST_AYUN.", fformat)
    data = foreign::read.dbf(raw)
  } # End EST_AYUN
  if (fuente == "INTEGRAN") {
    raw  = paste0(zipdir, "\\Estruct_aytto_cngmd2015_dbf\\Bases_de_datos\\INTEGRAN.", fformat)
    data = foreign::read.dbf(raw)
  } # End INTEGRAN
  if (fuente == "PER_APOY") {
    raw  = paste0(zipdir, "\\Estruct_aytto_cngmd2015_dbf\\Bases_de_datos\\PER_APOY.", fformat)
    data = foreign::read.dbf(raw)
  } # End PER_APOY

} # End of Estructura Ayuntamiento

### AYUNTAMIENTO - COMISIONES E INICIATIVAS -----------------------------------------------------------
else if (fuente == "ACT_AYUN" | fuente == "COM_AYUN" | fuente == "IN_TEASU") {
  url.comisiones  = paste0("http://www.beta.inegi.org.mx/contenidos/proyectos/censosgobierno/municipal/cngmd/2015/microdatos/m1/Com_ini_aytto_cngmd2015_", fformat, ".zip")
  utils::download.file(url.comisiones, temp.cngmd)
  utils::unzip(temp.cngmd, exdir = zipdir)
  
  if (fuente == "ACT_AYUN") {
    raw  = paste0(zipdir, "\\Com_ini_aytto_cngmd2015_dbf\\Bases_de_datos\\ACT_AYUN.", fformat)
    data = foreign::read.dbf(raw)
  } # End EST_AYUN
  if (fuente == "COM_AYUN") {
    raw  = paste0(zipdir, "\\Com_ini_aytto_cngmd2015_dbf\\Bases_de_datos\\COM_AYUN.", fformat)
    data = foreign::read.dbf(raw)
  } # End INTEGRAN
  if (fuente == "IN_TEASU") {
    raw  = paste0(zipdir, "\\Com_ini_aytto_cngmd2015_dbf\\Bases_de_datos\\IN_TEASU.", fformat)
    data = foreign::read.dbf(raw)
  } # End PER_APOY
  
} # End of Comisiones Ayuntamiento
